...
public class ConnectViaCode : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (FindObjectsOfType<ConnectViaCode>().Length>1)
        {
    ...
    private void WebSocketBLiveClientOnGift(SendGift sendGift)
    {
        ...
        var totalCoin = sendGift.giftNum * sendGift.price;
        Debug.Log(sb);
        EventCenter.Broadcast(EnumEventType.OnGiftReceived, sendGift.uid, sendGift.userName, sendGift.giftNum, sendGift.giftName,totalCoin);
    ...
    private void WebSocketBLiveClientOnDanmaku(Dm dm)
    {
        ...
        sb.Append(dm.msg);
        Debug.Log(sb);
        EventCenter.Broadcast(EnumEventType.OnDanMuReceived,dm.userName, dm.uid, "0", dm.msg);
    }
